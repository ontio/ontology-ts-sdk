...
export function encryptWithEcb(privateKey: string, publicKey: string, keyphrase: string, scryptParams: ScryptParams = DEFAULT_SCRYPT): string {
    let u160 = getSingleSigUInt160(publicKey);
    // console.log( "programHash: ", programHash );   
    ...
    // AES Encrypt
    let xor = hexXor(privateKey, derived1);
    let encrypted = CryptoJS.AES.encrypt(CryptoJS.enc.Hex.parse(privateKey), CryptoJS.enc.Hex.parse(derived2), { mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.NoPadding });
    //console.log( "encrypted: ", encrypted.ciphertext.toString() );
...
export function decryptWithEcb(encryptedKey: string, keyphrase: string,scryptParams: ScryptParams = DEFAULT_SCRYPT): string {
    let assembled = ab2hexstring(Bs58check.decode(encryptedKey));
