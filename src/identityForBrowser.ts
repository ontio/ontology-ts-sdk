...
export class IdentityForBrowser {
    /**
     * Import identity
    ...
    static async importIdentity(
        ...
        saltBase64: string,
        params?: ScryptParams
    ): Promise<IdentityForBrowser> {
        // create identity
        const identity = new IdentityForBrowser();
        const salt = Buffer.from(saltBase64, 'base64').toString('hex');
        const privateKey = await encryptedPrivateKey.decryptForBrowser(password, address, salt, params);
        if (!label) {
     ...
     * same as create, but for browser.
     *
     ...
     */
    static async create(privateKey: PrivateKey, keyphrase: string, label: string, params?: ScryptParams) {
        const identity = new IdentityForBrowser();
        identity.ontid = '';
        identity.label = label;
        identity.lock = false;
        ...
        const encryptedPrivateKey = await privateKey.encryptForBrowser(keyphrase, address, salt, params);
        // start from 1
    ...
    static parseJson(json: string): IdentityForBrowser {
        return IdentityForBrowser.parseJsonObj(JSON.parse(json));
    }
     ...
     */
    static parseJsonObj(obj: any): IdentityForBrowser {
        const id = new IdentityForBrowser();
        id.ontid = obj.ontid;
        id.label = obj.label;
        id.lock = obj.lock;
    ...
    async exportPrivateKey(password: string, params?: ScryptParams) {
        const encryptedKey = this.controls[0].encryptedKey;
        const address = this.controls[0].address;
        const salt = this.controls[0].salt;
        return await encryptedKey.decryptForBrowser(password, address, salt, params);
    }

    async signTransaction(password: string, tx: Transaction, params?: ScryptParams) {
        const pri = await this.exportPrivateKey(password, params);
