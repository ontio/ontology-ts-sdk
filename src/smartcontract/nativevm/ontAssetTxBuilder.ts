...
export function makeTransferTxV2(
    ...
    const contract = getTokenContract(tokenType);
    const params = buildNativeCodeScript(list);
    const tx: Transfer = makeNativeContractTx('transferV2', params, contract, gasPrice, gasLimit) as any;
    tx.tokenType = tokenType;
    tx.from = from;
    tx.to = to;
    tx.amount = amount;
...
export function makeTransferStateTxV2(
    ...
    const params = buildNativeCodeScript(list);
    const contract = getTokenContract(tokenType);
    const tx = makeNativeContractTx('transferV2', params, contract, gasPrice, gasLimit);
    if (payer) {
        tx.payer = payer;
    }
    return tx;
...
export function makeWithdrawOngTxV2(from: Address, to: Address, amount: number | string, payer: Address,
    ...
    const args = buildNativeCodeScript(list);
    const tx: Transfer = makeNativeContractTx(
        'transferFromV2', args, new Address(ONG_CONTRACT) , gasPrice, gasLimit) as any;
    tx.payer = payer;
    tx.tokenType = 'ONG';
    tx.from = from;
    tx.to = to;
 ...
 * @param to Receiver's address
 */
export function makeQueryAllowanceTxV2(asset: string, from: Address, to: Address): Transaction {
    asset = asset.toLowerCase();
    if (asset !== 'ont' && asset !== 'ong') {
        throw ERROR_CODE.INVALID_PARAMS;
    }
    ...
    list.push(struct);
    const params = buildNativeCodeScript(list);
    const tx = makeNativeContractTx('allowanceV2', params, new Address(contract), '0', '0');
    return tx;
}
 ...
 * @param address Address to query balance
 */
export function makeQueryBalanceTxV2(asset: string,  address: Address): Transaction {
    asset = asset.toLowerCase();
    if (asset !== 'ont' && asset !== 'ong') {
        throw ERROR_CODE.INVALID_PARAMS;
    }
    ...
    const params = hex2VarBytes(address.serialize());
    const tx = makeNativeContractTx('balanceOfV2', params, new Address(contract), '0', '0');
    return tx;
}
...
export function deserializeTransferTxV2(str: string): Transfer { // todo check deserializeTransferTxV2
    const tx: Transfer = Transaction.deserialize(str) as any;
    const code = tx.payload.serialize();
    const contractIndex1 = code.lastIndexOf('14' + '000000000000000000000000000000000000000');
    const contractIndex2 = code.lastIndexOf('14' + '0000000000000000000000000000000000000002');
